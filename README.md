...
├── Agent.md                  # Detailed schema description of merged_data.csv for the LLM.
├── prompt_template.txt       # The master prompt template for the LLM.
...
1.  Open `prompt_template.txt`. This file contains the full context the LLM needs.
2.  Copy its entire content and paste it into your preferred LLM interface (e.g., Gemini, ChatGPT).
3.  The template is designed to automatically include the schema from `Agent.md` and has a placeholder for the user prompt. **Manually replace the `{user_prompt}` placeholder at the bottom of the template with the content of your `user_prompt.txt` file.**
